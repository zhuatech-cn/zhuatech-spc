...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"processCode\":\"LINE-A-DIAMETER\",\"usl\":10.5,\"lsl\":9.5,\"mean\":10.05,\"sigma\":0.08,\"sampleSize\":30,\"ruleViolation\":false}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"processCode\":\"LINE-A-DIAMETER\",\"usl\":10.5,\"lsl\":9.5,\"mean\":10.4,\"sigma\":0.4,\"sampleSize\":10,\"ruleViolation\":true}"))
   .andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("STOP_PROCESS")).andExpect(jsonPath("$.data.actions").isNotEmpty());
 }
